...
const EditCouponPage = () => {
  ...
  useEffect(() => {
    const getCoupon = async () => {
      try {
        const res = await axios.get(`/v2/api/${process.env.REACT_APP_API_PATH}/admin/coupon/${id}`);
        if (res.data.success) {
          setCoupon(res.data.coupon);
        }
      } catch (error) {
        console.error("Error fetching coupon:", error);
        toast.error("獲取優惠券資料失敗");
      }
    };
    getCoupon();
  }, [id]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.put(`/v2/api/${process.env.REACT_APP_API_PATH}/admin/coupon/${id}`, {
        data: coupon,
      });
      if (res.data.success) {
        toast.success("更新成功");
